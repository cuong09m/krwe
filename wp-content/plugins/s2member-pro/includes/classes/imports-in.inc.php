						...
						if(!empty($_POST["ws_plugin__s2member_pro_import_users"]) && ($nonce = $_POST["ws_plugin__s2member_pro_import_users"]) && wp_verify_nonce($nonce, "ws-plugin--s2member-pro-import-users") && current_user_can("create_users"))
							{
								...
								if(isset($file) && is_resource($file) && !($imported = 0)) /* Only process if we have a resource. */
									{
										$configured_custom_reg_fields = array(); /* Initialize. */
										if($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["custom_reg_fields"])
											foreach(json_decode($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["custom_reg_fields"], true) as $field)
												if($field_var = preg_replace("/[^a-z0-9]/i", "_", strtolower($field["id"])))
													$configured_custom_reg_fields[] = $field_var;
										/**/
										while(($data = ((version_compare(PHP_VERSION, "5.3", ">=")) ? fgetcsv($file, 0, ",", '"', '"') : fgetcsv($file, 0, ",", '"'))) !== false)
											{
												...
												if(is_multisite() && c_ws_plugin__s2member_utils_conds::is_multisite_farm() && !is_main_site())
													{
														...
														/**/
														$custom_fields = array(); /* Initialize Custom Fields. */
														if(count($data) > 16) /* Now loop through Custom Fields. */
															for($i = 16; $i < count($data); $i = $i + 2) /* Skip over 2 fields at a time here. */
																if($field_var = preg_replace("/[^a-z0-9]/i", "_", strtolower(trim($data[$i], "^* \t\n\r\0\x0B"))))
																	if(in_array($field_var, $configured_custom_reg_fields)) /* Must be pre-configured. */
																		$custom_fields[$field_var] = maybe_unserialize($data[$i + 1]);
													}
												else /* Otherwise, we use the standardized format for importation.*/
													{
														...
														if(count($data) > 17) /* Now loop through Custom Fields. */
															for($i = 17; $i < count($data); $i = $i + 2) /* Skip over 2 fields at a time here. */
																if($field_var = preg_replace("/[^a-z0-9]/i", "_", strtolower(trim($data[$i], "^* \t\n\r\0\x0B"))))
																	if(in_array($field_var, $configured_custom_reg_fields)) /* Must be pre-configured. */
																		$custom_fields[$field_var] = maybe_unserialize($data[$i + 1]);
													}
												...
												/**/
												else if(is_multisite() && !is_super_admin($user_id) && ($user_id = c_ws_plugin__s2member_utils_users::ms_user_login_email_exists_but_not_on_blog($user_login, $user_email)))
													{
