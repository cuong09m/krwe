						...
						if(!($response = c_ws_plugin__s2member_pro_paypal_responses::paypal_form_api_validation_errors($attr)) || !empty($attr["register"]))
							{
								...
								/**/
								else /* Validation routines for Member Level checkout forms. This is the default functionality. */
									{
										...
										/**/
										else if($attr["rr"] !== "BN" && $GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["paypal_payflow_api_username"] && !in_array($attr["rp"]."-".$attr["rt"], array("1-W", "2-W", "1-M", "3-M", "1-Y"), TRUE))
											$response = array("response" => _x('Invalid Payflow® form configuration. Invalid "rt, rp, rr" attributes. Payflow® supports a specific set of recurring intervals. Pro Forms can be configured to charge: weekly, bi-weekly, monthly, quarterly, or yearly. Any other combination results in this error. This is a Payflow® limitation. Note that Payflow® does NOT support daily and/or bi-monthly billing.', "s2member-admin", "s2member"), "error" => true);
										/**/
