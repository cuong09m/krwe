...
<div class="FA_overall_container_title_nav FA_slider <?php the_slider_color();?>" style="<?php the_slider_width()?>" id="<?php echo $FA_slider_id;?>">	
	...
	<?php foreach ($postslist as $post):?>
		<li><a href="<?php the_permalink();?>" title="<?php the_title();?>"><?php if(get_post_meta($post->ID, 'wpcf-nation', true)) {?><?php echo get_post_meta($post->ID, 'wpcf-nation', true);?>: <?php }?><?php the_title();?></a></li>
	<?php endforeach;?>
	</ul>
</div>